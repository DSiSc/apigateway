...
func makeJSONRPCHandler(funcMap map[string]*RPCFunc, cdc *amino.Codec, logger log.Logger) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if request.ID == nil {
			...
			craftlog.Debug("HTTPJSONRPC received a notification, skipping... (please send a non-empty ID if you want to call a method)")
			return
		}
		if len(r.URL.Path) > 1 {
	...
	for {
		...
		default:
			...
			if request.ID == nil {
				...
				craftlog.Debug("WSJSONRPC received a notification, skipping... (please send a non-empty ID if you want to call a method)")
				continue
			}
...
// If some writes don't set it while others do, they may timeout incorrectly (https://github.com/DSiSc/apigateway/issues/553)
